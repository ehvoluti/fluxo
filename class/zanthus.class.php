...
class Zanthus{
	...
	public function leituraVendas($dtulvenda = "", $hrultvenda = "00:00:00", $processardia = false){
		...
		if($processardia){
			$this->dtmovto_ini = $dtulvenda;
			$this->dtmovto_fim = $dtulvenda;
			...
			$BUSCAR_MOVIMENTOS->addChild("MARCAR", "1", null);
		}else{			
		...
		foreach($result->QUERY->CONTENT->MOVIMENTOS->M00 AS $M00){			
			...
			// 158 = Sanguia
			// 154 = 
            if(in_array($numfuncao,array("158","148","154","155","151","159"))){
				continue;
			}else{
				$nenhumavenda = false;
			}
			...
			foreach($M00->ZAN_M45S->ZAN_M45 AS $M45){				
				...
				if(trim($funcao) == "147"){
					$cupomrelacionada = (string) $M01->M45BO;
					$cumpom = $cupomrelacionada;
					$status = "C";
				}
			}
			...
			foreach($M00->ZAN_M03S AS $ZAN_M03){
				foreach($ZAN_M03 as $iten){
					...
					$pdvitem->setcodproduto(str_pad($codproduto, 8, "0", STR_PAD_LEFT), TRUE);
			...
			foreach($M00->ZAN_M02S->ZAN_M02 AS $M02){
				...
				if(strlen(trim($cpfcnpj),0) == 0){	
					$cpfcnpj = (string) $M02->M02AM;
				}				
				$codfinaliz = (string) $M02->M02AI;
				$valorenvolvido = (string) $M02->M02AK;
				$valortroco = (string) $M02->M02AL;
				$valortotal = $valorenvolvido - $valortroco;
			...
			foreach($M00->ZAN_M04S->ZAN_M04 AS $M04){
				...
				$valorenvolvido = (string) $M04->M04AK;
				$valortroco = (string) $M04->M04AW;
				$valortotal = $valorenvolvido - $valortroco;
		...
		if($processardia && $this->temmovimento){
			$this->leituraVendas($dtulvenda, "00:00:00", true);
		}

		if($nenhumavenda){
			$this->marcarMovimentos();
	   	}
...
//			$this->arr_movimento = array();
	...
	public function importar_maparesumo(){
		$this->setconfigwebservice();
		$paramfiscal = objectbytable("paramfiscal", $this->codestabelec, $this->con);	
		...
		$BUSCAR_MOVIMENTOS->addChild("COD_LOJA", $this->codestabelec, null);
	
		$BUSCAR_MOVIMENTOS->addChild("DTA_INICIO", $this->dtmovto_ini."T00:00:00", null);
		$BUSCAR_MOVIMENTOS->addChild("DTA_FIM", $this->dtmovto_fim."T23:00:00", null);
		$BUSCAR_MOVIMENTOS->addChild("TIPO_DOCUMENTO", "RZ", null);
		...
		$arr_obj_ecf = object_array($ecf);

		$arr_ecf = [];
		foreach($arr_obj_ecf as $ecf){
			$arr_ecf[trim($ecf->getnumeroecf())] = $ecf;
		}	
		$arr_erro = [];
		$this->con->start_transaction();
		foreach($result->QUERY->CONTENT->MAPA_RESUMO[0] AS $m => $z_mapa){	
			foreach($z_mapa AS $z_ecf){
				$dtmovto = (string) $z_ecf->DATA;
				$dtmovto = substr($dtmovto,0,10);
				$caixa = (string) $z_ecf->NUM_ECF;
				$numeroecf = (string) $z_ecf->NUM_ECF;
				$cupominicial = (string) $z_ecf->NUM_INICIAL;
				$cupomfinal = (string) $z_ecf->NUM_FINAL;
				$gtinicial = (string) $z_ecf->GT_INICIAL;
				$gtfinal = (string) $z_ecf->GT_FINAL;
				$totalbruto = (string) $z_ecf->VAL_MOVIMENTO_DIA;
				$totalcanc = (string) $z_ecf->VAL_CANCELAMENTO;
				$cuponscancelados = 1;
				$itenscancelados = 1;
				$totaldescto = (string) $z_ecf->DESCONTO;
				$serieecf = (string) $z_ecf->NUM_SERIE_EQUIPAMENTO;
				$contreinicio = (string) $z_ecf->CONTADOR_REINICIO_OPERACAO;
				$contreduc = (string) $z_ecf->CONTADOR_REDUCAO_Z;

				$totalsubsticms = (string) $z_ecf->VAL_TRIBUTARIA;
				$totalisento = (string) $z_ecf->VAL_ISENTA;

				$ecf = $arr_ecf[trim($numeroecf)];

				$maparesumo = objectbytable("maparesumo", NULL, $this->con);
				$maparesumo->setcodestabelec($this->codestabelec);
				$maparesumo->setcaixa($caixa);
				$maparesumo->setnumeroecf($numeroecf);
				$maparesumo->setnummaparesumo(($paramfiscal->getnummaparesumo() == 0 ? 1 : $paramfiscal->getnummaparesumo()));
				...
				$maparesumo->setoperacaoini($cupominicial);
				$maparesumo->setoperacaofim($cupomfinal);
				$maparesumo->setgtinicial($gtinicial);
				$maparesumo->setgtfinal($gtfinal);
				$maparesumo->settotalbruto($totalbruto);
				$maparesumo->settotalcupomcancelado($totalcanc);
				...
				if(!$maparesumo->save()){
					...
					$arr_erro[] = "Não gravou o mapa resumo do caixa: $caixa, data: $dtmovto, total bruto: $totalbruto ";
					continue;
				}
				$codmaparesumo = $maparesumo->getcodmaparesumo();
				...
				if($totalsubsticms > 0){
					$maparesumoimposto = objectbytable("maparesumoimposto", NULL, $this->con);
					$maparesumoimposto->setcodmaparesumo($codmaparesumo);
					$maparesumoimposto->settptribicms("F");
					$maparesumoimposto->setaliqicms(0);
					$maparesumoimposto->settotalliquido($totalsubsticms);
					$maparesumoimposto->settotalicms(0);
					if(!$maparesumoimposto->save()){
						//$this->con->rollback();
						//return FALSE;
						$arr_erro[] = "Erro ao gravar o mapa $codmaparesumo para tributacao <b>F</b> ";						
					}
				}
				...
				if($totalisento > 0){
					$maparesumoimposto = objectbytable("maparesumoimposto", NULL, $this->con);
					$maparesumoimposto->setcodmaparesumo($codmaparesumo);
					$maparesumoimposto->settptribicms("I");
					$maparesumoimposto->setaliqicms(0);
					$maparesumoimposto->settotalliquido($totalisento);
					$maparesumoimposto->settotalicms(0);
					if(!$maparesumoimposto->save()){
						//$this->con->rollback();
						//return FALSE;
						$arr_erro[] = "Erro ao gravar o mapa $codmaparesumo para tributacao <b>I</b> ";
					}
				}

				foreach($z_ecf->ICMS[0] AS $z_trib){
					$aliqicms = $z_trib[0]->ALIQUOTA;
					$totalliquido = $z_trib[0]->VALOR_TOTAL;
					$totalicms = $z_trib[0]->VALOR_TRIBUTO;
					...
					$maparesumoimposto = objectbytable("maparesumoimposto", NULL, $this->con);
					$maparesumoimposto->setcodmaparesumo($codmaparesumo);
					$maparesumoimposto->settptribicms("T");
					...
					if(!$maparesumoimposto->save()){
						//$this->con->rollback();
						//return FALSE;
						$arr_erro[] = "Erro ao gravar o mapa $codmaparesumo para tributacao <b>T $aliqicms</b> ";
					}
				}
	...
	private function enviar_produto(){
		...
		while($temproduto){
			...
			$query .= "COALESCE((SELECT composicao.explosaoauto FROM composicao WHERE composicao.codproduto = produto.codproduto AND composicao.tipo = 'V' LIMIT 1),'N') AS explosao, ";
			...
			foreach($arr_pro as $produto){
				...
				if($produto["coddepto"] > 999){
					$produto["coddepto"] = substr($produto["coddepto"], -2) + 100;
				}
				...
				$MERCADORIA->addChild("CFOP", $produto["cfop"], null);
				...
				if($produto["explosao"] == "S"){
					$MERCADORIA->addChild("FLG_FORMA_DECOMPOSICAO", "1", null);
					...
					$query .= "WHERE composicao.codproduto = {$produto["codproduto"]} AND composicao.tipo = 'V' ";
					...
					foreach($arr_composicao as $composicao){
						...
						$PRODUTO_DECOMPOSICAOS = $REPLACE->addChild("PRODUTO_DECOMPOSICAOS");
						$PRODUTO_DECOMPOSICAOS = $PRODUTO_DECOMPOSICAOS->addChild("PRODUTO_DECOMPOSICAO");
						$PRODUTO_DECOMPOSICAOS->addChild("USUARIO");
						$PRODUTO_DECOMPOSICAOS->addChild("COD_LOJA", $this->pdvconfig->getestabelecimento()->getcodestabelec(), null);
						$PRODUTO_DECOMPOSICAOS->addChild("COD_MERCADORIA", $produto["codean"], null);
						$PRODUTO_DECOMPOSICAOS->addChild("TIPO_DECOMPOSICAO", 6, null);
						$PRODUTO_DECOMPOSICAOS->addChild("FORMA_DECOMPOSICAO", 1, null);
	...
	private function enviar_cliente($offset = 0){	
		echo script("console.log('Enviando clientes')");	
		$limit = 100;

		$query = "SELECT count(cliente.codcliente) ";
		$query .= "FROM cliente ";
		...
		$query .= "cidade.codoficial, cliente.codcidaderes, cliente.codpaisres, ";
		$query .= "	cliente.complementoent, cliente.limite1, cliente.debito1, cliente.limite2, cliente.debito2, ";
		$query .= "(SELECT nome FROM cliente WHERE codcliente = cliente.codempresa LIMIT 1) AS nomeempresa, ";
		$query .= "cliente.tipopreco, cliente.descfixo, cliente.ufres, cliente.senha ";
		$query .= "FROM cliente ";
		$query .= "LEFT JOIN cidade ON (cliente.codcidaderes = cidade.codcidade) ";
		$query .= "INNER JOIN statuscliente ON (cliente.codstatus = statuscliente.codstatus) ";		
		...
		foreach($arr_cliente as $row){
			$i++;
			setprogress($i / $total_registros * 100, "Exportando Clientes: ".$i." de ".$total_registros, TRUE);			

			$CLIENTE = $CLIENTES->addChild("CLIENTE");

			$CLIENTE->addChild("COD_CLIENTE", removeformat($row["codcliente"]), null);
			$CLIENTE->addChild("DES_CLIENTE", utf8_encode(substr($row["nome"],0,36)), null);
			...
			$CLIENTE->addChild("NUMERO", substr(trim(manter_numeros($row["numerores"])),0,6), null);
			$CLIENTE->addChild("COMPL_ENDERECO", $row["complementores"], null);
			$CLIENTE->addChild("DES_BAIRRO", utf8_encode($row["bairrores"]), null);
			...
			echo script("console.log('{$row["nome"]}')");
		}
		...
		return $this->enviar_cliente($offset += 100);
	}
