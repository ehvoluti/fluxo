...
final class SpedFiscal extends Sped{
	...
	protected $perin = false;

	protected $estabelecimento;
	protected $paramestoque;
	...
	protected $totalizador_difal = array(); // Totais do difal por UF
	protected $gerar_bloco_k = FALSE;
	protected $gerar_cadastroprodutocompleto = FALSE;
	protected $arr_ajustes;	//array contendo os ajuste de estoque escriturados em periodos anteriores
	protected $datafinalajuste;	//data final do periodo anterior de estoque escriturado
			function __construct($con){
		parent::__construct($con);

		if(param("SISTEMA", "CODIGOCW", $this->con) === "1543"){
			$this->perin = true;
		}
	}
	...
	private function cupom_para_notafiscal($cupom){
		...
		if(strlen($numnotafis) === 0 || $numnotafis === 0){
			$numnotafis = (int) str_replace("I", "", $cupom["cupom"]);
		}
		if(strlen($serie) === 0 || $serie === 0){
			$serie = $cupom["caixa"];
		}

		$codparceiro = $cupom["codcliente"];
		if($codparceiro == $this->paramestoque->getcodclientevendapdv()){
			$codparceiro = null;
		}
		...
		$notafiscal = array(
			...
			"totaldesoneracao" => $cupom["totaldesoneracao"],
			"totalgnre" => 0,
	...
	public function gerar(){
		...
		$this->paramestoque = objectbytable("paramestoque", $this->estabelecimento->getcodemitente(), $this->con);
		...
		if(!$this->gerar_tudozerado && $this->gerar_notafiscal){
			$query = "SELECT DISTINCT notafiscal.* FROM notafiscal ";
			...
			$query .= "	AND (((notafiscal.operacao IN ('CP','DF','TE','EF','EC') OR (notafiscal.operacao IN ('DC') AND notafiscal.emissaopropria = 'N')) AND notafiscal.dtentrega BETWEEN '".$this->datainicial."' AND '".$this->datafinal."') ";
			$query .= "	 OR  ((notafiscal.operacao NOT IN ('CP','DF','TE','DC','EF','EC') OR (notafiscal.operacao IN ('DC') AND notafiscal.emissaopropria = 'S'))  AND notafiscal.dtemissao BETWEEN '".$this->datainicial."' AND '".$this->datafinal."')) ";
...
//			$query .= " AND notafiscal.gerapiscofins = 'S' ";
			...
			if(count($arr_idnotafiscal) > 0){
				...
				$query .= "	AND (operacao NOT IN ('".implode("','", $arr_operacao_sem_composicao)."') OR composicao IN ('N','P')) ";
...
//			$arr_idnotafiscal = array();
		...
		if(!$this->gerar_tudozerado && $this->gerar_notafiscal){
			$query = "SELECT notadiversa.*, itnotadiversa.*, natoperacao.codconta ";
		...
		if(!$this->gerar_tudozerado && $this->gerar_notafiscal){
			$query = "SELECT notafrete.*, natoperacao.codconta, cidade_orig.codoficial AS codmunicipioorig, cidade_dest.codoficial AS codmunicipiodest ";
			$query .= "FROM notafrete ";
			$query .= "LEFT JOIN v_parceiro ON ('T' = v_parceiro.tipoparceiro AND notafrete.codtransp = v_parceiro.codparceiro) ";
			$query .= "LEFT JOIN cidade as cidade_orig ON (v_parceiro.codcidade = cidade_orig.codcidade) ";
			$query .= "LEFT JOIN estabelecimento ON (notafrete.codestabelec = estabelecimento.codestabelec) ";
			$query .= "LEFT JOIN cidade as cidade_dest ON (estabelecimento.codcidade = cidade_dest.codcidade) ";
		...
		if(!$this->gerar_tudozerado && $this->gerar_cupom){
			...
			foreach($arr as $row){
				if(strlen($row["codecf"]) == 0){
					$_SESSION["ERROR"] = "O mapa resumo de chave <b>".$row["codmaparesumo"]."</b> se encontra sem um ECF associado. Para processeguir com a gera&ccedil;&atilde;o do arquivo &eacute; necess&aacute;rio relacionar um ECF ao mapa resumo.<br><a onclick=\"$.messageBox('close');openProgram('MapaResumo','codmaparesumo=".$row["codmaparesumo"]."')\">Clique aqui</a> para abrir o mapa resumo.";
					return FALSE;
				}else{
		...
		$arr_idcupom = array();
		if($this->gerar_cupom){
			$query  = "SELECT cupom.*, (CASE WHEN ecf.equipamentofiscal = 'SAT' THEN '' ELSE maparesumo.codmaparesumo::TEXT END) AS codmaparesumo, ecf.equipamentofiscal ";
			$query .= "FROM cupom ";
			...
			if($this->gerar_contmatic){
				$query .= "  AND (cupom.status = 'A' OR cupom.status = 'C') ";
			}else{
				$query .= "  AND (cupom.status IN ('A', 'I') OR SUBSTR(cupom.chavecfe, 21, 2) = '65') ";
			}
			foreach($this->arr_filtro as $coluna => $valor){
				if($coluna == "dtentregaini"){
			...
			foreach($arr as $row){
				...
				if(strlen($codmaparesumo) === 0 && $equipamentofiscal === "SAT"){
					if((strlen($row["chavecfe"]) > 0 || $row["status"] === "I") && (strlen($row["chavecfe"]) === 0 || !isset($arr_cupom_chavecfe[$row["chavecfe"]]))){
						$codmaparesumo = "SAT";
						if(strlen($row["chavecfe"]) > 0){
							$arr_cupom_chavecfe[$row["chavecfe"]] = true;
						}
					}else{
						continue;
					}
		...
		if(count($arr_idcupom) > 0){
			$query = "SELECT (CASE WHEN ecf.equipamentofiscal = 'SAT' THEN 'SAT' ELSE COALESCE(maparesumo.codmaparesumo::TEXT, 'SAT') END) AS codmaparesumo, ";
			$query .= "  itcupom.idcupom, itcupom.codproduto, SUM(itcupom.quantidade) AS quantidade, itcupom.preco, itcupom.desconto, ";
			$query .= "  itcupom.acrescimo, SUM(round(itcupom.valortotal,2)) AS valortotal, itcupom.aliqicms, ";
		...
		foreach($this->arr_cupom as $codmaparesumo => $arr_cupom){
			foreach($arr_cupom as $idcupom => $cupom){
				if($cupom["status"] === "A" && (!is_array($cupom["itcupom"]) || count($cupom["itcupom"]) == 0)){
		...
		if(is_array($this->arr_cupom["SAT"])){
			foreach($this->arr_cupom["SAT"] as $i => $cupom){
				...
				// Nota fiscal consumidor eletronico (NFC-e)
				if($modelo === "65" || $cupom["status"] === "I"){
		...
		if(param("NOTAFISCAL", "CUSTOSPED", $this->con) == 1){
			...
			$query_custosemimp .= " AND ps.codproduto = produtoestab.codproduto AND ps.codestabelec IN (SELECT codestabelec FROM estabelecimento WHERE codestabelecfiscal = {$this->estabelecimento->getcodestabelec()}) ORDER BY data DESC  LIMIT 1)::numeric(12,2) ";
		}

		/*Geração do bloco K esta atrelado a data informada no parametro fiscal da loja*/
		$paramfiscal = objectbytable("paramfiscal", $this->estabelecimento->getcodestabelec(), $this->con);
		$datainicioblocok = $paramfiscal->getdatainicioblocok();
		if(is_null($datainicioblocok)){
			$gerarajustes = FALSE;
			$this->gerar_bloco_k = FALSE;
		}else{
			$mesiniajuste = date("m/Y", strtotime($datainicioblocok));
			$mesiniescrit = date("m/Y", strtotime($this->datainicial));
			$gerarajustes = ($mesiniescrit > $mesiniajuste);
		}

		$arr_produto_inventario = array();
		if($this->gerar_inventario || $this->gerar_bloco_k){
			$query_saldo_H = ($this->gerar_inventario ? "sum(saldo(codestabelec,codproduto,'{$datainventario}')) AS saldo, " : "");
			$query_saldo_K = ($this->gerar_bloco_k ? "sum(saldo(codestabelec,codproduto,'{$this->datafinal}')) AS saldoatual, " : "");
			$query = "SELECT codproduto, {$query_saldo_H} {$query_saldo_K} ".$query_custosemimp." AS custosemimp ";
			$query .= "FROM produtoestab ";
			$query .= "WHERE codestabelec IN (SELECT codestabelec FROM estabelecimento WHERE codestabelecfiscal = ".$this->estabelecimento->getcodestabelec().") ";
			$query .= "	AND produtoestab.disponivel = 'S' ";
			$query .= "GROUP BY 1, produtoestab.custosemimp";

			$res = $this->con->query($query);
			$arr = $res->fetchAll(2);
			foreach($arr as $row){
				if(($row["saldo"] > 0 || $row["saldoatual"] > 0)){
					$arr_codproduto[] = $row["codproduto"];
					$arr_produto_inventario[$row["codproduto"]] = $row;
				}
			}

			if($this->gerar_bloco_k){
				if($gerarajustes){
					$arr_diasmes = array("31","28","31","30","31","30","31","31","30","31","30","31");
					$query = "SELECT codtpdocto FROM tipodocumento WHERE gerarblocok = 'S'";
					$res = $this->con->query($query);
					$arr_tipodocumento = $res->fetchAll(2);
					if(count($arr_tipodocumento) > 0){
						foreach($arr_tipodocumento as $tipodocumento){
							$codtpdoc .= "{$tipodocumento["codtpdocto"]},";
						}
						$mesdata = date("m",$this->datafinal);
						$dataini = date("Y-m-d", strtotime("-1 month", strtotime($this->datainicial)));
						$datafin = date("Y-m-d", strtotime("-{$arr_diasmes[$mesdata - 1]} days", strtotime($this->datafinal)));
						$this->datafinalajuste = $datafin;
						$codtpdoc = substr($codtpdoc,0, strlen($codtpdoc)-1);
						$query = "SELECT codproduto, SUM(CASE WHEN tipo = 'S'  THEN quantidade*(-1) ELSE quantidade END) AS quantidade "
								."FROM movimento "
								."WHERE dtmovto BETWEEN '{$dataini}' AND '{$datafin}' "
								."AND codtpdocto IN ({$codtpdoc}) AND codestabelec = {$this->estabelecimento->getcodestabelec()} "
								."GROUP BY 1";
						$res = $this->con->query($query);
						$this->arr_ajustes = $res->fetchAll(2);
						foreach($this->arr_ajustes  as $ajuste){
							$arr_codproduto[] = $ajuste["codproduto"];
						}
					}
				}
			}
		...
		if(count($arr_codproduto) > 0){
			$where_produto = ($this->gerar_cadastroprodutocompleto ? "" : " WHERE codproduto IN (".implode(",", $arr_codproduto).")");
			$res = $this->con->query("SELECT * FROM produto {$where_produto} ORDER BY descricaofiscal");
			$arr = $res->fetchAll(2);
			foreach($arr as $row){
		...
		foreach($arr_produto_inventario as $produto_inventario){
			...
			$this->arr_produto[$produto_inventario["codproduto"]]["saldoatual"] = $produto_inventario["saldoatual"];
		...
		foreach($this->arr_fornecedor as $fornecedor){
			if(strlen(trim($fornecedor["cpfcnpj"])) == 0 && $fornecedor["codpais"] == "01058"){
		...
		foreach($this->arr_notafiscal as $i => $notafiscal){
			if(strlen($notafiscal["cupom"]) > 0 && $notafiscal["operacao"] != "DC"){
				foreach($notafiscal["itnotafiscal"] as $j => $itnotafiscal){
		...
		if(compare_date($this->datainicial, "2019-01-01", "Y-m-d", ">=")){
			$this->progresso("Gerando bloco B");
			$this->bloco_B();
		}
	...
	public function gerar_bloco_k($b){
		if(is_bool($b)){
			$this->gerar_bloco_k = $b;
		}
	}

	public function gerar_cadastroprodutocompleto($b){
		if(is_bool($b)){
			$this->gerar_cadastroprodutocompleto = $b;
		}
	}
		...
		if(strlen($notafiscal["idnotacomplemento"]) > 0){
			...
			if(in_array($this->modelo($notafiscal), array("01", "1B", "04", "55", "65"))){
				foreach($notafiscal["itnotafiscal"] as $itnotafiscal){
					...
					$natoperacao = $this->arr_natoperacao[$itnotafiscal["natoperacao"]];
					if((in_array(substr($codcst, 1, 2), array("00", "20")) && $natoperacao["aproveitarcreditodebitoicms"] == "S") || (!in_array(substr($codcst, 1, 2), array("00", "20")) && $natoperacao["aprovicmsprop"] == "S")){
					...
					$vl_ipi = $this->valor_totalipi($itnotafiscal);
					...
					if(!in_array(substr($codcst, 1, 2), array("70", "60", "10"))){
						...
						$arr[$codcst][$natoperacao][$aliqicms]["totalipi"] += $vl_ipi;
					}else{
						...
						$arr[$orig."60"][$natoperacao][$aliqicms]["totalipi"] += $vl_ipi;
					}
	...
	// Escrituracao e apuracao do ISS
	protected function bloco_B(){
		$this->registro_B001();
		$this->registro_B990();
	}
	...
	protected function registro_0000(){
		...
		$registro = array(
			...
			// 1 - Outros
			"IND_ATIV" => ($this->estabelecimento->gettipoatividade() === "D" ? "0" : "1")
		);
	...
	protected function registro_0001(){
		...
		$this->registro_0200();
		$this->registro_0300();
		if($this->gerar_0400 && $this->gerar_contmatic){
			$this->registro_0400();
		}
		$this->registro_0450();
		$this->registro_0460();
	...
	protected function registro_0150(){
		...
		foreach($this->arr_notafiscal as $notafiscal){
			...
			if(!in_array($notafiscal["status"], array("C", "I", "D")) && !in_array($modelo, array("65", "2D"))){
...
//					$arr_codparceiro[] = "T".$notafiscal["codtransp"];
		...
		foreach($arr_codparceiro as $codparceiro){
			switch(substr($codparceiro, 0, 1)){ // Verifica qual o parceiro da nota
				case "C": // Busca dados do cliente
					...
					$nome = strlen($cliente["razaosocial"]) > 0 ? $cliente["razaosocial"] : $cliente["nome"];
	...
	protected function registro_0200(){
		foreach($this->arr_produto as $produto){
			...
			if(strlen($produto["cest"]) > 0){
				$cest = removeformat($produto["cest"]);
			}

			if(!in_array(substr($classfiscal["codcst"],1,2),array("10","60","70"))){
				$cest = "";
			}
	...
	protected function registro_0220($embalagem){
		...
		$registro = array(
			...
			"UNID_CONV" => ((float)$embalagem["quantidade"] > 0 ? $unidade["sigla"] : "UN"),
			...
			"FAT_CONV" => ((float)$embalagem["quantidade"] > 0 ? $this->valor_decimal($embalagem["quantidade"], 6) : $this->valor_decimal("1", 6))
		);
		$this->escrever_registro($registro);
	}
	...
	protected function registro_0400(){
		...
		foreach($_arr_natoperacao as $natoperacao){
			$registro = array(
				...
				"COD_NAT" => $this->valor_natoperacao($natoperacao["natoperacao"]),
	...
	protected function registro_0500(){
		...
		foreach($this->arr_notafiscal AS $notafiscal){
			foreach($notafiscal["itnotafiscal"] AS $itnotafiscal){
				$natoperacao = $this->arr_natoperacao[$itnotafiscal["natoperacao"]];
				if(strlen($natoperacao["codconta"]) > 0){
	...
	// Abertura do bloco B [nivel 1]
	protected function registro_B001(){
		$registro = array(
	...
	// Encerramento do bloco B [nivel 1]
	protected function registro_B990(){
		$registro = array(
			// Texto fixo contento "B990"
			"REG" => "B990",
			// Quantidade de linhas do bloco B
			"QTD_LIN_B" => 2
		);
		$this->escrever_registro($registro);
	}
	...
	protected function registro_C100(){
		...
		foreach($this->arr_notafiscal as $notafiscal){
			...
			$aux_totalliquido = $notafiscal["totalbruto"] + $notafiscal["totalfrete"] + $notafiscal["totalacrescimo"] - $notafiscal["totaldesconto"] +  $notafiscal["totalipi"] +  $notafiscal["totalicmssubst"];
			$aux_totaldesoneracao = $notafiscal["totaldesoneracao"];
			if($aux_totalliquido != $notafiscal["totalliquido"]){
				$dif = ($aux_totalliquido - $aux_totaldesoneracao) - $notafiscal["totalliquido"];
				//if((string)($aux_totalliquido - $aux_totaldesoneracao) == (string)$notafiscal["totalliquido"]){
				if($dif >= -0.99 && $dif <= 0.99){
					$notafiscal["totaldesconto"] += $notafiscal["totaldesoneracao"];
				}
			}

			if($this->perin && $notafiscal["operacao"] === "CP"){
				$notafiscal["totalbruto"] += $notafiscal["totalicmssubst"] + $notafiscal["totalipi"];
			}

			if(in_array($notafiscal["natoperacao"], array("5.929", "6.929"))){
				$cod_sit = "08";
				$notafiscal["totalliquido"] = null;
				$notafiscal["totalbruto"] = null;
			}
			...
			$registro = array(
				...
				// Valor total do IPI
				"VL_IPI" => ($vl_ipi == 0) ? null : $this->valor_decimal($vl_ipi, 2),
				// Valor total do PIS
			...
			if($ind_emit == "1" || strlen($notafiscal["chavenfe"]) == 0 || $this->gerar_contmatic || $this->perin){
				$this->registro_C105();
				$this->registro_C110();
				$this->registro_C111();
				$this->registro_C112();
				...
				if($notafiscal["totalfcpufdest"] > 0 || $notafiscal["totalicmsufdest"] > 0 || $notafiscal["totalicmsufremet"] > 0){
					$this->registro_C101($notafiscal);
				}
				$this->registro_C190($notafiscal);
		...
		foreach($this->arr_notacomplemento as $notacomplemento){
			...
			$natoperacao = $this->arr_natoperacao[$notacomplemento["natoperacao"]];
			$operacaonota = $this->arr_operacaonota[$natoperacao["operacao"]];
			// Verifica qual o indicador de pagamento
			$arr_lancamento = $this->arr_lancamento[$notafiscal["idnotafiscal"]];
			if(count($arr_lancamento) > 0){
				if(count($arr_lancamento) == 1 && $arr_lancamento[0]["dtemissao"] == $arr_lancamento[0]["dtvencto"]){
			...
			$registro = array(
				...
				// Valor total do IPI
				"VL_IPI" => ($cod_sit == "06" && $notacomplemento["totalipi"] > 0) ? $this->valor_decimal($notacomplemento["totalipi"], 2) : null,
				// Valor total do PIS
		...
		foreach($this->arr_notadiversa AS $notadiversa){
			if(in_array($this->valor_natoperacao($notadiversa["tipodocumentofiscal"]), array("55"))){
				$registro = array(
					...
					// Valor total do IPI
					"VL_IPI" => $notadiversa["totalipi"] > 0 ? $this->valor_decimal($notadiversa["totalipi"], 2) : null,
					// Valor total do PIS
	...
	protected function registro_C101($notafiscal){
		$registro = array(
			// Texto fixo contendo "C101"
			"REG" => "C101",
			// Valor total relativo ao Fundo de Combate à Pobreza (FCP) da UF de destino
			"VL_FCP_UF_DEST" => $this->valor_decimal($notafiscal["totalfcpufdest"], 2),
			// Valor total do ICMS Interestadual para a UF de destino
			"VL_ICMS_UF_DEST" => $this->valor_decimal($notafiscal["totalicmsufdest"], 2),
			// Valor total do ICMS Interestadual para a UF do remetente
			"VL_ICMS_UF_REM" => $this->valor_decimal($notafiscal["totalicmsufremet"], 2)
		);

		switch($notafiscal["tipoparceiro"]){
		...
		$this->totalizador_difal[$uf_parceiro]["totalfcpufdest"] += $notafiscal["totalfcpufdest"];
		$this->totalizador_difal[$uf_parceiro]["totalicmsufdest"] += $notafiscal["totalicmsufdest"];
		$this->totalizador_difal[$uf_parceiro]["totalicmsufremet"] += $notafiscal["totalicmsufremet"];
		$this->totalizador_difal[$uf_parceiro]["emitdest"] = "D";

		$cidade = $this->arr_cidade[$this->estabelecimento->getcodcidade()];
		$this->totalizador_difal[$cidade["uf"]]["totalfcpufdest"] += 0;
		$this->totalizador_difal[$cidade["uf"]]["totalicmsufdest"] += $notafiscal["totalicmsufdest"];
		$this->totalizador_difal[$cidade["uf"]]["totalicmsufremet"] += $notafiscal["totalicmsufremet"];
		$this->totalizador_difal[$cidade["uf"]]["emitdest"] = "E";
		$this->escrever_registro($registro);
	}
	...
	protected function registro_C170($notafiscal){
		...
		if(in_array($this->modelo($notafiscal), array("01", "1B", "04", "55")) || $this->gerar_contmatic){
			...
			foreach($notafiscal["itnotafiscal"] as $itnotafiscal){
				...
				$csticms = $this->csticms($itnotafiscal);
				...
				$aux_totalliquido = $itnotafiscal["totalbruto"] + $itnotafiscal["totalfrete"] + $itnotafiscal["totalacrescimo"] - $itnotafiscal["totaldesconto"] +  $itnotafiscal["totalipi"] +  $itnotafiscal["totalicmssubst"];
				$aux_totaldesoneracao = $itnotafiscal["valordesoneracao"];

				if($aux_totalliquido != $itnotafiscal["totalliquido"]){
					$dif = ($aux_totalliquido - $aux_totaldesoneracao) - $itnotafiscal["totalliquido"];
					//if((string)($aux_totalliquido - $aux_totaldesoneracao) == (string)$itnotafiscal["totalliquido"]){
					if($dif >= -0.99 && $dif <= 0.99){
						$itnotafiscal["totaldesconto"] += $itnotafiscal["valordesoneracao"];
					}
				}

				if($this->perin && $notafiscal["operacao"] === "CP"){
					$itnotafiscal["totalbruto"] += $itnotafiscal["totalicmssubst"] + $itnotafiscal["totalipi"];
				}
				...
				if(compare_date($this->datainicial, "2019-01-01", "Y-m-d", ">=")){
					// Valor do abatimento nao tributado e nao comercial
					$registro["VL_ABAT_NT"] = "";
				}
				...
				if($this->gerar_contmatic){
					...
					$registro = array_merge($registro, array(
						...
						// Valor do Frete
						"VR_FRETE" => $this->valor_decimal($itnotafiscal["totalfrete"], 2),
						// Valor do seguro indicado no documento fiscal
						"VL_SEG" => $this->valor_decimal(0, 2),
						// Valor de outras despesas acessórias
						"VL_OUT_DA" => $this->valor_decimal($itnotafiscal["totalacrescimo"], 2),
						// IPI Não Aproveitável, (informar 4 quando for ipi não aproveitável , caso contrario deixar em branco)
						"IND_IPI" => null,
						// Uso interno
						"USO_INTERNO1" => null,
						// Uso interno
						"USO_INTERNO2" => null,
						// Valor total da Base de Cálculo do ICMS Interestadual para a UF de destino
						"BC_ICMS_DEST" => $this->valor_decimal($itnotafiscal["basecalcufdest"], 2),
						// Informar a alíquota interna da UF de Destino para o produto
						"ALIQ_UF_DEST" => $this->valor_decimal($itnotafiscal["aliqicmsufdest"], 2),
						// Informar a alíquota interestadual das UF envolvidas
						"ALIQ_INTERESTADUAL" => $this->valor_decimal($itnotafiscal["aliqicmsinter"], 2),
						// Informar o Percentual do ICMS relativo ao Fundo de Combate à Pobreza (FCP) na UF de Destino
						"FCP" => $this->valor_decimal($itnotafiscal["aliqfcpufdest"], 2),
						// Valor do ICMS relativo ao Fundo de Combate à Pobreza da UF de Destino
						"ICMS_FCP" => $this->valor_decimal($itnotafiscal["valorfcpufdest"], 2),
						// Valor do ICMS Interestadual para a UF de Destino
						"ICMS_DEST" => $this->valor_decimal($itnotafiscal["valoricmsufdest"], 2),
						// Valor do ICMS interestadual para a UF do Remetente.
						"ICMS_REM" => $this->valor_decimal($itnotafiscal["valoricmsufremet"], 2)
					));
				}
				$this->escrever_registro($registro);
				$this->registro_C171();
		...
		foreach($arr as $codcst => $arr2){
			foreach($arr2 as $natoperacao => $arr3){
				foreach($arr3 as $aliqicms => $arr4){
					if(in_array($notafiscal["natoperacao"], array("5.929", "6.929"))){						
						$arr4["totalliquido"] = null;
						$arr4["totalbaseicms"] = null;
					}
	...
	protected function registro_C400(){
		foreach($this->arr_ecf as $ecf){
			if($ecf["equipamentofiscal"] !== "ECF" && !$this->gerar_contmatic){
				continue;
			}
...
//				if($cupom["numeroecf"] != $ecf["numeroecf"]){
	...
	protected function registro_C460($maparesumo = null, $cupom = null){
		if(!is_null($maparesumo)){
			$codmaparesumo = ($maparesumo === "SAT" ? "SAT" : $maparesumo["codmaparesumo"]);

			foreach($this->arr_cupom[$codmaparesumo] as $cupom){
				if($cupom["status"] != "A"){
					continue;
				}
				...
				if($this->gerar_contmatic){
					$registro_contimatic = array(
						// Número de Série do equipamento SAT
						"NR_SAT" => substr($cupom["chavecfe"], 22, 9),
						// Chave do Cupom Fiscal Eletrônico
						"CHV_CFE" => $cupom["chavecfe"],
						// Número Cupom Cancelamento
						"CFE_CANC" => $cupom["status"] == "C" ? $cupom["cupom"] : ""
					);
				}
	...
	protected function registro_C470($cupom){
		foreach($cupom["itcupom"] as $itcupom){
			...
			if($this->gerar_contmatic && $cupom["equipamentofiscal"] == "SAT"){
				$valortotal = $itcupom["valortotal"] + $itcupom["desconto"] - $itcupom["acrescimo"];
			}else{
				$valortotal = $itcupom["valortotal"];
			}
			...
			$registro = array(
				...
				"UNID" => $unidade["sigla"],
				// Valor total do item
				"VL_ITEM" => $this->valor_decimal($valortotal, 2),
				// Codigo da situacao tributaria, conforme a tabela indicada no item 4.3.1
			...
			if($this->gerar_contmatic){
				...
				if($cupom["equipamentofiscal"] !== "SAT"){
					$itcupom["acrescimo"] = 0;
					$itcupom["desconto"] = 0;
				}
...
//					"VL_ICMS" => $this->valor_decimal($row["totalicms"],2),
	...
	protected function registro_C860(){
		foreach($this->arr_maparesumo as $maparesumo){
			$ecf = $this->arr_ecf[$maparesumo["codecf"]];
			if($ecf["equipamentofiscal"] !== "SAT"){
				continue;
			}
			$registro = array(
				...
				"DOC_INI" => $maparesumo["operacaoini"],
				// Numero do documento final
				"DOC_FIM" => $maparesumo["operacaofim"]
			);
			$this->escrever_registro($registro);
			$this->registro_C890($maparesumo);
		}
	}

	// Resumo diario do CF-E-SAT (codigo 59) por equipamento SAT-CF-E
	protected function registro_C890($maparesumo){
		$arr_analitico = array();
		foreach($this->arr_cupom[$maparesumo["codmaparesumo"]] as $cupom){
			foreach($cupom["itcupom"] as $itcupom){
				$csticms = $this->csticms($itcupom);
	...
	protected function registro_D100(){
		foreach($this->arr_notafrete as $notafrete){
			...
			$registro = array(
				...
				"COD_MUN_ORIG" => str_pad($notafrete["codmunicipioorig"],7,"0",STR_PAD_LEFT),
				// Código do município de destino, conforme a tabela IBGE (Preencher com 9999999, se Exterior)
				"COD_MUN_DEST" => str_pad($notafrete["codmunicipiodest"],7,"0",STR_PAD_LEFT)
			);
			$this->escrever_registro($registro);
	...
	protected function registro_E001(){
		...
		$this->registro_E200();
		$this->registro_E300();
		if($this->estabelecimento->gettipoatividade() === "D"){
			$this->registro_E500();
		}
	}
	...
	protected function registro_E110(){
		...
		foreach($this->arr_notafiscal as $notafiscal){
			foreach($notafiscal["itnotafiscal"] as $itnotafiscal){
				...
				$natoperacao = $this->arr_natoperacao[$itnotafiscal["natoperacao"]];
				if((in_array(substr($codcst, 1, 2), array("00", "20")) && $natoperacao["aproveitarcreditodebitoicms"] == "S") || (!in_array(substr($codcst, 1, 2), array("00", "20")) && $natoperacao["aprovicmsprop"] == "S")){
					$valor_icms = round($itnotafiscal["totalicms"], 2);
				}else{
					$valor_icms = 0;
				}
				$natoperacao = $this->valor_natoperacao($itnotafiscal["natoperacao"]);
		...
		foreach($this->arr_cupom as $codmaparesumo => $arr_cupom){
			foreach($arr_cupom as $cupom){
				foreach($cupom["itcupom"] as $itcupom){
					...
					$valortotal = (in_array($itcupom["tptribicms"], array("T", "R")) ? $itcupom["valortotal"] : 0);

					if($valortotal > 0){
						if(strlen($cupom["chavecfe"]) > 0){
							$debito += round($valortotal * ($aliqicms / 100), 2);
						}else{
							$debito += $valortotal * ($aliqicms / 100);
						}
					}
				}
			}
	...
	protected function registro_E210($uf){
		...
		foreach($this->arr_notafiscal as $notafiscal){
			...
			if(!in_array($operacaonota["operacao"], array("DC", "PR", "RC", "VD", "TS", "NC","RF","EC"))){
				continue;
			}
			...
			foreach($notafiscal["itnotafiscal"] as $itnotafiscal){
				...
				$cfop_devolucao = array("1410", "1411", "1414", "1415", "1660", "1661", "1662", "2410", "2411", "2414", "2415", "2660", "2661", "2662");

				if(in_array($cfop, $cfop_devolucao)){
					$devolucao += $itnotafiscal["totalicmssubst"];
				}

				if(in_array(substr($cfop, 0, 1), array("1", "2")) && !in_array($cfop, $cfop_devolucao)){
	...
	protected function registro_E300(){
		foreach($this->totalizador_difal AS $uf => $difal){
			$registro = array(
				...
				"DT_FIM" => $this->valor_data($this->datafinal)
			);
			$this->escrever_registro($registro);

			$this->registro_E310($difal);
		}
	}

	// APURAÇÃO DO ICMS DIFERENCIAL DE ALÍQUOTA – UF ORIGEM/DESTINO EC 87/15. [nivel 2]
	protected function registro_E310($difal){

		if($difal["emitdest"] == "E"){
			$totalicmsuf = $difal["totalicmsufremet"];
			$totalfcpufdest = 0;
		}else{
			$totalicmsuf = $difal["totalicmsufdest"];
			$totalfcpufdest = $difal["totalfcpufdest"];
		}

		$registro = array(
			//1 Texto fixo contendo "E310"
			"REG" => "E310",
			// Indicador de movimento:
			...
			//2
			"IND_MOV_DIFAL" => $this->valor_decimal(0),
			//3 Valor do "Saldo credor de período anterior – ICMS Diferencial de Alíquota da UF de Origem/Destino"
			"VL_SLD_CRED_ANT_DIFAL" => $this->valor_decimal(0),
			//4 Valor total dos débitos por "Saídas e prestações com débito do ICMS referente ao diferencial de alíquota devido à UF do Remetente/Destinatário
			"VL_TOT_DEBITOS_DIFAL" => $this->valor_decimal($totalicmsuf, 2),
			//5 Valor Total dos ajustes "Outros débitos ICMS Diferencial de Alíquota da UF de Origem/Destino" " e “Estorno de créditos ICMS Diferencial de Alíquota da UF de Origem/Destino
			"VL_OUT_DEB_DIFAL" => $this->valor_decimal(0),
			//6 Valor total dos débitos FCP por "Saídas e prestações
			"VL_TOT_DEB_FCP" => $this->valor_decimal(0),
			//7 Valor total dos créditos do ICMS referente ao diferencial de alíquota devido à UF dos Remetente/ Destinatário
			"VL_TOT_CREDITOS_DIFAL" => $this->valor_decimal(0),
			//8 Valor total dos créditos FCP por Entradas
			"VL_TOT_CRED_FCPA" => $this->valor_decimal($totalicmsuf, 2),
			//9 Valor total de Ajustes "Outros créditos ICMS Diferencial de Alíquota da UF de Origem/Destino" e “Estorno de débitos ICMS Diferencial de Alíquota da UF de Origem/Destino
			"VL_OUT_CRED_DIFAL" => $this->valor_decimal(0),
			//10 Valor total de Saldo devedor ICMS Diferencial de Alíquota da UF de Origem/Destino antes das deduções
			"VL_SLD_DEV_ANT_DIFAL" => $this->valor_decimal($totalicmsuf, 2),
			//11 Valor total dos ajustes "Deduções ICMS Diferencial de Alíquota da UF de Origem/Destino
			"VL_DEDUCOES_DIFAL" => $this->valor_decimal(0),
			// Valor recolhido ou a recolher referente a FCP e Imposto do Diferencial de Alíquota da UF de Origem/Destino (10-11)
			"VL_RECOL" => $this->valor_decimal(0),
			...
			"VL_SLD_CRED_TRANSPORTAR" => $this->valor_decimal(0),
			// Valores recolhidos ou a recolher, extraapuração
			"DEB_ESP_DIFAL" => $this->valor_decimal($totalfcpufdest, 2),
			// Valor total dos ajustes "Outros débitos FCP" e "Estorno de créditos FCP"
			"VL_OUT_DEB_FCP" => $this->valor_decimal(0),
			// Valor total dos créditos FCP por Entradas.
			"VL_TOT_CRED_FCP" => $this->valor_decimal(0),
			// Valor total de Ajustes "Outros créditos FCP" e "Estorno de débitos FCP".
			"VL_OUT_CRED_FCP" => $this->valor_decimal(0),
			// Valor total de Saldo devedor FCP antes das deduções.
			"VL_SLD_DEV_ANT_FCP" => $this->valor_decimal($totalfcpufdest, 2),
			// Valor total das deduções "FCP".
			"VL_DEDUÇÕES_FCP" => $this->valor_decimal(0),
			// Valor recolhido ou a recolher referente ao FCP (18-19).
			"VL_RECOL_FCP" => $this->valor_decimal($totalfcpufdest, 2),
			// Saldo credor a transportar para o período seguinte referente ao FCP.
			"VL_SLD_CRED_TRANSPORTAR_FCP" => $this->valor_decimal(0),
			// Valores recolhidos ou a recolher, extra-apuração - FCP.
			"DEB_ESP_FC" => $this->valor_decimal(0)
		);

		$this->escrever_registro($registro);
		$this->registro_E316($difal);
	}

	// PERÍODO DE APURAÇÃO DO ICMS DIFERENCIAL DE ALÍQUOTA – UF ORIGEM/DESTINO EC 87/15
	protected function registro_E316($difal){
		if($difal["emitdest"] == "E"){
			$codrec = "100-4";
			$totalicmsuf = $difal["totalicmsufremet"];
			$totalfcpufdest = 0;
		...
		$registro = array(
			...
			"VL_OR" => $this->valor_decimal($totalicmsuf + $totalfcpufdest, 2),
			...
			"MES_REF" => substr($this->valor_data($this->datafinal), 2, 6)
		);
		$this->escrever_registro($registro);
	}
		...
		foreach($this->arr_notafiscal as $notafiscal){
			if(in_array($this->modelo($notafiscal), array("01", "1B", "04", "55"))){
				foreach($notafiscal["itnotafiscal"] as $itnotafiscal){
					...
					$ipi = $this->arr_ipi[$produto["codipi"]];
					$natoperacao = $this->valor_natoperacao($itnotafiscal["natoperacao"]);
					$codcst = $this->cstipi($itnotafiscal);
					$totalipi = $this->valor_totalipi($itnotafiscal);
					$arr_ipi[$natoperacao][$codcst]["totalbaseipi"] += ($totalipi > 0 ? $itnotafiscal["totalbruto"] - $itnotafiscal["totaldesconto"] + $itnotafiscal["totalacrescimo"] : 0);
					$arr_ipi[$natoperacao][$codcst]["totalipi"] += $totalipi;
				}
			}
		}
		...
		foreach($this->arr_notafiscal as $notafiscal){
			if(in_array($this->modelo($notafiscal), array("01", "1B", "04", "55"))){
				foreach($notafiscal["itnotafiscal"] as $itnotafiscal){
					$cfop = substr(removeformat($itnotafiscal["natoperacao"]), 0, 4);
					if(in_array(substr($cfop, 0, 1), array("5", "6"))){
						$debito += $this->valor_totalipi($itnotafiscal);
					}
					if(in_array(substr($cfop, 0, 1), array("1", "2", "3"))){
						$credito += $this->valor_totalipi($itnotafiscal);
					}
				}
			}
		}
	...
	protected function registro_K001(){
		$registro = array(
			...
			// 0 - Bloco com dados informados
			// 1 - Bloco sem dados informados
			"IND_MOV" => ($this->gerar_bloco_k ? "0" : "1" )
		);
		$this->escrever_registro($registro);
		if($this->gerar_bloco_k){
			$this->registro_K100();
	...
	// Periodo de apuracao do ICMS/IPI [nivel 2]
	protected function registro_K100(){
		$registro = array(
			"REG" => "K100",
			// Data inicial das informacoes contidas no arquivo
			"DT_INI" => $this->valor_data($this->datainicial),
	...
	// Estoque escriturado [nivel 3]
	protected function registro_K200(){
		foreach($this->arr_produto as $produto){
			if($produto["saldoatual"] > 0){
				$registro = array(
					"REG"		=> "K200",											// identificador do registro
					"DT_EST"	=> $this->valor_data($this->datafinal),				// data do estoque final
					"COD_ITEM"	=> $produto["codproduto"],							// codigo do item (campo 02 do registro 0200)
					"QTD"		=> $this->valor_decimal($produto["saldoatual"], 3),	// Quantidade em estoque
	...
	// Correcao de apontamento (ajuste de estoque) - escriturado [nivel]
	protected function registro_K280(){
		foreach ($this->arr_ajustes as $ajuste){
	...
	protected function registro_1010(){
		$query  = "SELECT itnotafiscal.codproduto, cidade.codoficial, sum(itnotafiscal.totalliquido) AS totalliquido ";
		$query .= "from itnotafiscal ";
		$query .= "INNER JOIN notafiscal on (notafiscal.idnotafiscal = itnotafiscal.idnotafiscal) ";
		...
		$query .= "INNER JOIN cidade ON (v_parceiro.codcidade = cidade.codcidade) ";
		$query .= "WHERE notafiscal.operacao = 'PR' AND v_parceiro.uf = '{$this->estabelecimento->getuf()}' ";
		$query .= " AND notafiscal.codestabelec IN (SELECT codestabelec FROM estabelecimento WHERE codestabelecfiscal={$this->estabelecimento->getcodestabelec()}) ";
		$query .= "	AND notafiscal.dtemissao BETWEEN '{$this->datainicial}' AND '{$this->datafinal}' ";
		foreach($this->arr_filtro as $coluna => $valor){
			if($coluna == "dtentregaini"){
				$query .= "	AND notafiscal.dtemissao >= '".$valor."' ";
			}elseif($coluna == "dtentregafim"){
				$query .= "	AND notafiscal.dtemissao <= '".$valor."' ";
			}else{
				if($coluna == "operacao"){
		...
		$query .= "group by itnotafiscal.codproduto, cidade.codoficial ";
		$res = $this->con->query($query);
		$arr_1400 = $res->fetchAll();
		...
		if(compare_date($this->datainicial, "2019-01-01", "Y-m-d", ">=")){
			// Reg. 1960 - Possui informações GIAF1? (Sim/Nao)
			$registro["IND_GIAF1"] = "N";
			// Reg. 1960 - Possui informações GIAF3? (Sim/Nao)
			$registro["IND_GIAF3"] = "N";
			// Reg. 1960 - Possui informações GIAF4? (Sim/Nao)
			$registro["IND_GIAF4"] = "N";
		}
		$this->escrever_registro($registro);
		...
		if(sizeof($arr_1400) > 0){
			$this->registro_1400($arr_1400);
		}
	}
	...
	protected function registro_1400($arr_1400){

		foreach($arr_1400 AS $row){
			$registro = array(
				// Texto fixo contendo "1210
				"REG" => "1400",
				// Código do item (Tabela própria da unidade da federação (Tabela de Itens UF Índice de Participação dos Municípios) ou campo 02 do Registro 0200
				"COD_ITEM_IPM" => "SPDIPAM11",
				// Código do Município de origem/destino
				"MUN" => $row["codoficial"],
				// Valor mensal correspondente ao município
				"VALOR" => $this->valor_decimal($row["totalliquido"], 2)
			);
		}
		$this->escrever_registro($registro);
	}
