...
final class Connection extends PDO{
	...
	private $error;
	private $transaction;
	private $create_log = false;
	private $log_file;

	function __construct($param_1 = NULL, $param_2 = NULL, $param_3 = NULL){
		if($this->create_log){
			if(!is_dir("../temp/log-connection")){
				mkdir("../temp/log-connection");
			}
			$filename = date("Y-m-d___H-i-s")."___".rand().".log";
			$this->log_file = fopen("../temp/log-connection/{$filename}", "w+");
		}
	...
	function query($statement){
		if($this->create_log){
			fwrite($this->log_file, date("Y-m-d   H:i:s")."\r\n{$statement}\r\n\r\n");
		}
